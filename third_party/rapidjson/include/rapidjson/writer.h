...
public:
	...
	Writer& JsonString(const Ch* str, SizeType length, bool copy = false) {
		Prefix(kStringType);
		WriteString(str, length, false);
		return *this;
	}

	Writer& SetDocJson(const Ch* str, SizeType length, bool copy = false) {
		Prefix(kStringType);
		WriteString(str, length, false);
		return *this;
	}
	...
	Writer& JsonString(const Ch* str) { return JsonString(str, internal::StrLen(str)); }
...
protected:
	...
	void WriteString(const Ch* str, SizeType length, bool needQuoted)  {
...
#undef Z16
		...
		for (const Ch* p = str; p != str + length; ++p) {
			if (needQuoted == true && (sizeof(Ch) == 1/** || *p < 256**/) && escape[(unsigned char)*p])  {
				stream_.Put('\\');
