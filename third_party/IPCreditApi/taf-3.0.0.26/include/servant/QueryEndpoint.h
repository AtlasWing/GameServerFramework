...
public:
	...
	void findObjectById4All(const string& name, set<EndpointInfo> & setActive, set<EndpointInfo> & setInactive);

	void findObjectByStation(const string& name, const string & sStation, set<EndpointInfo> & vecActive, set<EndpointInfo> & vecInactive);

    void findObjectByIdInSameSet(const string& name,const string& sSetId, bool sync);
