...
/**@file domparser.h @brief DOM parsing interface.
*
* Use this interface to parse XML data with the document
* object model.
*/

#ifndef __COM_INTEL_XML_PARSE_DOM_DOMPARSER_H__
#define __COM_INTEL_XML_PARSE_DOM_DOMPARSER_H__
...
/**
  * Implements the DOM parser interface.
  *
  * This class can be used to instantiate a well-formed or non-validating
  * parser by setting the member flag.
  *
  */
class DOM_EXPORT DOMParser
{
public:
    /**
     * Desctructor
...
public:
    ...
    /**
     * Parses the Reader object to an XML document.
     *
     * @descr @n Set the Reader object explicitly to provide the method with access
     * to the data and set the @e uri parameter to the source URI for the
     * parser.
     *
     ...
     *                 to the XML file that needs to be parsed
     *
     * @return The parsed document.
     * @return In case of an error, produces DOMParseException, IOException, ValidationException.
     *
     */
    virtual Document* parse(iutil::Reader* reader,const ixml::XMLChar* uri=NULL) = 0;

    /**
     * Parses the XML file specified by URI to an XML document.
     *
     * @descr @n The external resourc bease URI (Uniform Resource Identifier) will be extacted
     * from the uri parameter unless it is set by setExternalResourceBaseURI().
     *
     * @note  At present, the format of "file:///" and "http:///"
     * is supported.
     *
     * @param uri    - the constant char pointer to the native string with the path
     *                 to the XML file that needs to be parsed
     *
     * @return The parsed document. In case of an error, returns DOMParseException,
     * IOException, ValidationException.
     *
     */
    virtual Document* parse(const ixml::XMLChar* uri) = 0;
    ...
    *
    */
    virtual Document* parse(ixml::StreamSource* source) = 0;
    ...
    /**
     * Configures the parser to strip whitespace nodes.
     *
     * @descr @n A @e whitespace node is a Text node with the value containing only
     * the following: @c &\#x20;, @c &\#x9; @c &\#xD
     * or @c &\#xA;.
     *
    ...
    /**
     * Checks whether the parser strips whitespace nodes.
     *
     ...
     * the following setting should be ignored:
     * @code <?xml ... encoding= ...?> @endcode
     *
    ...
    /**
     * Gets the current encoding as an enumerator.
     *
    ...
    /**
     * Configures the parser to reject HTML or XHTML documents.
     *
     ...
     *
     * @descr @n If this method returns eturns TRUE, a DOCTYPE declaration
     ...
     * parse() methods, DOMParser intercepts such URI
   ...
   /**
    * Parses with schema validation support.
    *
    ...
    */
    ...
    /**
     * Gets the Schema object that is used to validate the XML instance.
     *
    ...
    * DOM parser resolves system identifiers and opens connections
    * to entities on its own.
    *
    * Applications can register a new or a different entity resolver
    * in the middle of a parse operation, and the DOM parser must begin using
